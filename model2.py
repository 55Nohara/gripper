...
class TDNN_Dataset():
    def __init__(self, csvWithPath, fileNameData, fileNameLabel, t_input):
        ...
        data_ = np.diff(data_ , axis =1)  ###dim : N * (T-1) * 3
        # data_[:, :, 0] /= 6
        # data_[:, :, 1] /= 4
        # data_[:, :, 2] /= 30
        
        fin_datasets = np.empty((num_objects * T_scale , t_input, 3)) ### (N* T) * t_input * 3
        ...
        for i in range(num_objects): ### i:object number
            ...
            for k in range(t_input):
                ...
                fin_datasets[i*T_scale + k] = init_datasets[i * t_input + k]
