...
const float sensorConst = (60.0*0.46) / 1000.0;  // センサ計算に使用する係数を予め求めておく。水圧センサ用のコンバート用関数の作成。
...
void sendSensorData(){
  ...
  int averageNum = 5;
  int16_t readFromAds;
  int16_t read_StrainData_1;
  int16_t read_StrainData_2;
  float volts;
  float volts_strain_1;
  float volts_strain_2;
  float pressureData;
  float strainData_1;
  float strainData_2;
  ...
  read_StrainData_1 = ads.readADC_SingleEnded(2); //channel 1
  volts_strain_1 = ads.computeVolts(read_StrainData_1);
  read_StrainData_2 = ads.readADC_SingleEnded(3); //channel 1
  volts_strain_2 = ads.computeVolts(read_StrainData_2);
  //}
  pressureData = 6.895*(volts - 0.5)*(150.0/(0.8*5.0)); 
  strainData_1 = volts_strain_1;//計算式
  strainData_2 = volts_strain_2;
  Serial.print(pressureData, 4);
  Serial.print(',');
  Serial.print(strainData_1, 4);
  Serial.print(',');
  Serial.print(strainData_2, 4);
  Serial.print('\n'); Serial.flush();
